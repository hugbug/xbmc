...
void CResolutionUtils::FindResolutionFromWhitelist(float fps, int width, int height, bool is3D, RESOLUTION &resolution)
{
  ...
  for (const auto& mode : indexList)
  {
    ...
    // allow macroblock alignment / padding errors (e.g. 1080 mod16 == 8 or 2176 mod32 == 16)
    if (((height == info.iScreenHeight && width <= info.iScreenWidth + 16) ||
         (width == info.iScreenWidth && height <= info.iScreenHeight + 16)) &&
        (info.dwFlags & D3DPRESENTFLAG_MODEMASK) == (curr.dwFlags & D3DPRESENTFLAG_MODEMASK) &&
